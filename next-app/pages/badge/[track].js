/**
 * Track Badge Page - Shows individual badge for a completed track
 */

import { useEffect, useState } from 'react';
import { useRouter } from 'next/router';
import Head from 'next/head';
import Link from 'next/link';
import { getTrack, isTrackCompleted } from '../../src/tracks';
import useKoanProgress from '../../src/hooks/useKoanProgress';

export default function TrackBadge() {
  const router = useRouter();
  const { track: trackId } = router.query;
  const { progress } = useKoanProgress();
  const [isDownloading, setIsDownloading] = useState(false);
  const [completionDate, setCompletionDate] = useState('');

  const track = trackId ? getTrack(trackId) : null;
  const isComplete = track ? isTrackCompleted(track.id, progress) : false;

  useEffect(() => {
    const today = new Date().toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
    setCompletionDate(today);
  }, []);

  if (!router.isReady) {
    return null;
  }

  if (!track) {
    return (
      <div className="min-h-screen bg-gray-950 text-gray-100 py-12 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-3xl font-bold mb-4">Track Not Found</h1>
          <Link href="/badges" className="text-orange-500 hover:text-orange-400">
            ← Back to Badges
          </Link>
        </div>
      </div>
    );
  }

  const shareText = `🎉 I just earned the ${track.name} badge! Master your PySpark skills through interactive exercises. #PySpark #DataEngineering #Learning`;
  const shareUrl = typeof window !== 'undefined' ? window.location.origin : 'https://spark-koans.vercel.app';
  const ogImageUrl = `https://spark-koans.vercel.app${track.badge.ogImage}`;

  const shareOnTwitter = () => {
    const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(shareUrl)}`;
    window.open(url, '_blank', 'width=550,height=420');
  };

  const shareOnLinkedIn = () => {
    const url = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${shareUrl}/badge/${trackId}`)}`;
    window.open(url, '_blank', 'width=550,height=500');
  };

  const downloadBadge = async () => {
    setIsDownloading(true);
    try {
      const html2canvas = (await import('html2canvas')).default;
      const badge = document.getElementById('achievement-badge');

      if (!badge) {
        alert('Badge element not found');
        return;
      }

      const canvas = await html2canvas(badge, {
        backgroundColor: '#111827',
        scale: 2,
        logging: false,
      });

      canvas.toBlob((blob) => {
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.download = `${trackId}-badge.png`;
        link.href = url;
        link.click();
        URL.revokeObjectURL(url);
      });
    } catch (error) {
      console.error('Failed to download badge:', error);
      alert('Failed to download badge. Please try again.');
    } finally {
      setIsDownloading(false);
    }
  };

  return (
    <>
      <Head>
        <title>{track.name} Badge - PySpark Koans</title>
        <meta name="description" content={`${track.description} - Achievement badge`} />

        <meta property="og:title" content={`${track.name} Badge Earned!`} />
        <meta property="og:description" content={track.description} />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={`https://spark-koans.vercel.app/badge/${trackId}`} />
        <meta property="og:image" content={ogImageUrl} />
        <meta property="og:image:width" content="1200" />
        <meta property="og:image:height" content="630" />

        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`${track.name} Badge Earned!`} />
        <meta name="twitter:description" content={track.description} />
        <meta name="twitter:image" content={ogImageUrl} />
      </Head>

      <div className="min-h-screen bg-gray-950 text-gray-100 py-12 px-4">
        <div className="max-w-4xl mx-auto">
          <Link href="/badges" className="text-orange-500 hover:text-orange-400 mb-8 inline-block">
            ← Back to All Badges
          </Link>

          {!isComplete ? (
            <div className="bg-gray-900 border border-gray-800 rounded-lg p-12 text-center">
              <h1 className="text-3xl font-bold mb-4">Keep Going!</h1>
              <p className="text-gray-400 mb-8">
                Complete all required koans to earn the {track.name} badge.
              </p>
              <Link
                href="/koans/1"
                className="inline-block bg-orange-600 hover:bg-orange-700 text-white px-6 py-3 rounded-lg transition-colors"
              >
                Continue Learning
              </Link>
            </div>
          ) : (
            <>
              {/* Achievement Badge */}
              <div className="flex justify-center mb-8">
                <div id="achievement-badge" className="max-w-md">
                  <img
                    src={track.badge.image}
                    alt={track.badge.title}
                    className="w-full h-auto drop-shadow-2xl"
                  />
                  <p className="text-gray-400 text-center mt-6">
                    Completed on {completionDate}
                  </p>
                </div>
              </div>

              {/* Download & Share Section */}
              <div className="bg-gray-900 border border-gray-800 rounded-lg p-8 text-center">
                <h2 className="text-2xl font-bold mb-4">Download & Share Your Achievement</h2>
                <p className="text-gray-400 mb-6">
                  Download your {track.name} badge or share on social media
                </p>

                <div className="mb-6">
                  <button
                    onClick={downloadBadge}
                    disabled={isDownloading}
                    className="inline-flex items-center gap-2 bg-orange-600 hover:bg-orange-700 disabled:bg-gray-700 text-white font-semibold px-8 py-3 rounded-lg transition-colors"
                  >
                    {isDownloading ? (
                      <>
                        <svg className="animate-spin h-5 w-5" fill="none" viewBox="0 0 24 24">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        Generating...
                      </>
                    ) : (
                      <>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg>
                        Download Badge (PNG)
                      </>
                    )}
                  </button>
                </div>

                <p className="text-sm text-gray-500 mb-4">Or share directly:</p>
                <div className="flex flex-wrap justify-center gap-4">
                  <button
                    onClick={shareOnTwitter}
                    className="flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg transition-colors"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                    </svg>
                    Share on Twitter
                  </button>
                  <button
                    onClick={shareOnLinkedIn}
                    className="flex items-center gap-2 bg-blue-700 hover:bg-blue-800 text-white px-6 py-3 rounded-lg transition-colors"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                    </svg>
                    Share on LinkedIn
                  </button>
                </div>
              </div>
            </>
          )}
        </div>
      </div>
    </>
  );
}
